...
confirm($Fc="",$Df=false){return" onclick=\"".($Df?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($Fc?" (' + $Fc + ')":"")."');\"";}function
js_escape($J){return
addcslashes($J,"\r\n'\\/");}function
...
upload_error($n){$ke=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Nepodařilo se nahrát soubor.'.($ke?" ".sprintf('Maximální povolená velikost souboru je %sB.',$ke):""):'Soubor neexistuje.');}function
...
enum_input($z,$Ua,$d,$q,$ac=null){global$o;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$sa);$c=(isset($ac)?"<label><input type='$z'$Ua value='$ac'".((is_array($q)?in_array($ac,$q):$q===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($sa[1]as$k=>$b){$b=stripcslashes(str_replace("''","'",$b));$fb=(is_int($q)?$q==$k+1:(is_array($q)?in_array($k+1,$q):$q===$b));$c.=" <label><input type='$z'$Ua value='".($k+1)."'".($fb?' checked':'').'>'.h($o->editVal($b,$d)).'</label>';}return$c;}function
input($d,$q,$K){global$S,$o,$v;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$_e=($v=="mssql"&&$d["auto_increment"]);if($_e&&!$_POST["save"]){$K=null;}$ba=(isset($_GET["select"])||$_e?array("orig"=>'původní'):array())+$o->editFunctions($d);$Ua=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($tf,$Rb){$Bc=($Rb==1||(!$Rb&&'cs'=='fr')?0:('cs'=='sl'&&(!$Rb||$Rb>2)?1:0)+((!$Rb||$Rb>=5)&&ereg('cs|sk|ru|sl|pl','cs')?2:1));return
...
q($b);}function
check_sqlite_name($f){global$g;$ze="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($ze)\$~",$f)){$g->error=sprintf('Prosím použijte jednu z koncovek %s.',str_replace("|",", ",$ze));return
false;}return
true;}function
...
set_schema($La){global$g,$S,$Ka;$c=$g->query("SET search_path TO ".idf_escape($La));foreach(types()as$z){if(!isset($S[$z])){$S[$z]=0;$Ka['Uživatelské typy'][]=$z;}}return$c;}function
...
reset($G);}function
support($rb){return
ereg("view|drop_col|variables|status",$rb);}$v="oracle";$S=array();$Ka=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Řetězce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binární'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$S+=$b;$Ka[$e]=array_keys($b);}$mb=array();$fc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$ba=array("length","lower","round","upper");$ib=array("avg","count","count distinct","max","min","sum");$Qb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$la["mssql"]="MS SQL";if(isset($_GET["mssql"])){$_c=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Kc,$r=""){echo'<p class="tabs">';$Ma=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Kc)){$Ma["view"]='Pozměnit pohled';}else{$Ma["create"]='Pozměnit tabulku';}if(isset($r)){$Ma["edit"]='Nová položka';}foreach($Ma
...
selectColumnsPrint($P,$B){global$ba,$ib;print_fieldset("select",'Vypsat',$P);$k=0;$Ee=array('Funkce'=>$ba,'Agregace'=>$ib);foreach($P
...
selectLengthPrint($xb){if(isset($xb)){echo"<fieldset><legend>".'Délka textů'."</legend><div>",'<input name="text_length" size="3" value="'.h($xb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$v;static$Fc=0;restart_session();$R="sql-".($Fc++);$Wa=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$Wa[$_GET["db"]][]=$j;return" <a href='#$R' onclick=\"return !toggle('$R');\">".'SQL příkaz'."</a><div id='$R' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Wa[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
';if($Vb!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$Vb){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvořit novou tabulku'."</a>\n";$E=tables_list();if(!$E){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($E);$Ma=array();foreach($E
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Ye[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($S[$e]==254){$b="<code>$b</code>";}}if(isset($Ma[$e])&&!$B[$Ma[$e]]){$w="edit=".urlencode($Ma[$e]);foreach($I[$Ma[$e]]as$Lc=>$pa){$w.="&where".urlencode("[".bracket_escape($Lc)."]")."=".urlencode($a[$pa]);}$b="<a href='".h(ME.$w)."'>$b</a>";}}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Žádné řádky.')."\n";}function
...
edit_type($e,$d,$Y,$fa=array()){global$Ka,$S,$mb,$db;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($S[$d["type"]])?array():array($d["type"]))+$Ka+($fa?array('Cizí klíče'=>$fa):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($Y,$d["collation"]).'</select>',($mb?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($mb,$d["unsigned"]).'</select>':''),($fa?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist($db,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Lb?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($Lb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $Je, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.2' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.2' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($od==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.2' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Lb;}function
...
as$e=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($e)."<td><a href='".h(ME.'trigger='.urlencode($m).'&name='.urlencode($e))."'>".'Změnit'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($m).'">'.'Přidat trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databáze',"",array(),DB);$wb=array();$Pe=array();$f="adminer_schema";$Oe=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$Oe,$sa,PREG_SET_ORDER);foreach($sa
...
<input type="hidden" name="token" value="',$N,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobrazit pouze chyby')."\n";print_fieldset("webfile",'Ze serveru',$_POST["webfile"]);$nd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$nd[]=".$e";}}echo
...
as$f=>$d){if(!isset($d["privileges"][$ra?"update":"insert"])||$o->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ga=$_POST["referer"];if($_POST["insert"]){$ga=($ra?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ga)){$ga=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $s"),$ga,'Položka byla smazána.');}else{$r=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$r[idf_escape($f)]=($ra?"\n".idf_escape($f)." = $b":$b);}}if($ra){if(!$r){redirect($ga);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$r),"\nWHERE $s"),$ga,'Položka byla aktualizována.');}else{$i=insert_into($m,$r);$ae=($i?last_id():0);queries_redirect($ga,sprintf('Položka%s byla vložena.',($ae?" $ae":"")),$i);}}}$Fa=$o->tableName(table_status($m));page_header(($ra?'Upravit':'Vložit'),$n,array("select"=>array($m,$Fa)),$Fa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($s){$P=array();foreach($p
...
';if($p){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo'<input type="submit" name="insert" value="'.($ra?'Uložit a pokračovat v editaci':'Uložit a vložit další')."\">\n";}}echo($ra?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$p?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if($m==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Bd?html_select("Engine",array(""=>"(".'úložiště'.")")+$Bd,$a["Engine"]):""),' ',($Y&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'porovnávání'.")")+$Y,$a["Collation"]):""),' <input type="submit" value="Uložit">
...
',(support("comment")?checkbox("","",$Lb,'Komentář',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Lb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložit">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$N,'">
';if(support("partitioning")){$Sd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdělit podle',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$de,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddíly: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Sd||!$a["partition_by"]?" class='hidden'":""),'>
...
as$t){if(count($_)==1||$t!=""){if(!create_database($t,$_POST["collation"])){$Yd=false;}$vb=$t;}}queries_redirect(ME."db=".urlencode($vb),'Databáze byla vytvořena.',$Yd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$n,array(),DB);$Y=collations();$f=DB;$kc=null;if($_POST){$f=$_POST["name"];$kc=$_POST["collation"];}elseif(DB!=""){$kc=db_collation(DB,$Y);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$ka){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ka,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
';if($Va){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$N,'">
</form>
';}elseif(isset($_GET["event"])){$kb=$_GET["event"];$Md=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Ed=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($kb),substr(ME,0,-1),'Událost byla odstraněna.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Md)&&isset($Ed[$_POST["STATUS"]])){$be="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($kb!=""?'Událost byla změněna.':'Událost byla vytvořena.'),queries(($kb!=""?"ALTER EVENT ".idf_escape($kb).$be.($kb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$be)."\n".$Ed[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($kb!=""?'Pozměnit událost'.": ".h($kb):'Vytvořit událost'),$n);$a=array();if($_POST){$a=$_POST;}elseif($kb!=""){$G=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($kb));$a=reset($G);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Md,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Ed,$a["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložit">
';if($kb!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$N,'">
</form>
...
<input type="submit" value="Uložit">
';if($Qa!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($Va){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$N,'">
</form>
';}elseif(isset($_GET["sequence"])){$_b=$_GET["sequence"];if($_POST&&!$n){$w=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($_b),$w,'Sekvence byla odstraněna.');}elseif($_b==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$w,'Sekvence byla vytvořena.');}elseif($_b!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($_b)." RENAME TO ".idf_escape($_POST["name"]),$w,'Sekvence byla změněna.');}else{redirect($w);}}page_header($_b!=""?'Pozměnit sekvenci'.": ".h($_b):'Vytvořit sekvenci',$n);$a=array("name"=>$_b);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="submit" value="Uložit">
';if($_b!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$N,'">
</form>
';}elseif(isset($_GET["type"])){$Xc=$_GET["type"];if($_POST&&!$n){$w=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Xc),$w,'Typ byl odstraněn.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$w,'Typ byl vytvořen.');}}page_header($Xc!=""?'Pozměnit typ'.": ".h($Xc):'Vytvořit typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
';if($Xc!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložit'."'>\n";}echo'<input type="hidden" name="token" value="',$N,'">
</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$k=0;foreach($lb
as$bb=>$ka){echo'<th>'.($bb!="*.*"?"<input name='objects[$k]' value='".h($bb)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Uc=>$sc){foreach((array)$oa[$Uc]as$zc=>$_a){echo"<tr".odd()."><td".($sc?">$sc<td":" colspan='2'").' lang="en" title="'.h($_a).'">'.h($zc);$k=0;foreach($lb
